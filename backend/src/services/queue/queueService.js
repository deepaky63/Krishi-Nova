...
export async function list(filter, user) { if (user.role === 'staff' && !user.assignedCentreIds.some((id) => id.toString() === filter.centreId)) throw forbidden(); return QueueEntry.find(filter).populate('bookingId farmerId').sort({ priorityOverride: -1, checkedInAt: 1 }); }
